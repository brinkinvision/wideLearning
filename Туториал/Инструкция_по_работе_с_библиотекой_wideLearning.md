...
![1_dense](img/modified_dense/1.png#center)
...
![2_dense](img/modified_dense/2.png#center)
...
![3_dense](img/modified_dense/3.png#center)
...
# 2 Сверточный слой
Для примера работы использован набор данных MNIST и структура нейронной сети LeNet-5: [lenet_5](https://github.com/SlinkoIgor/Neural_Networks_and_CV/blob/master/module05_mnist_conv.ipynb)
...
```
В LeNet-5 в первом сверточном слое используется 6 сверточных ядер 3x3 с шагом 1. Число вычислительных операций для слоя равно:
```
wdl.count_conv_operations(28, 28, 1, 3, 1, 6)
```
![1_conv](img/discrete_conv/1.png)
...
Теперь сравним ресурсоемкость вычислений при использовании четырех сверточных ядер размерности 14x14 с шагом 7:
```
wdl.count_conv_operations(28, 28, 1, 14, 7, 4)
```
![2_conv](img/discrete_conv/2.png)
...
```
Сгенерируем горизонтальное ядро размера 14x14, состоящее из значений диапазона от -7 до 7 с шагом в 2:
```
wdl.horizontal(14, [-7,-5,-3,-1,1,3,5,7])
```
![3_conv](img/discrete_conv/3.png)

Аналогичным образом сгенерируем вертикальное ядро:
```
wdl.vertical(14, [-7,-5,-3,-1,1,3,5,7])
```
![4_conv](img/discrete_conv/4.png)
...
```
![5_conv](img/discrete_conv/5.png)
...
Чтобы посмотреть в визуально удобном виде установленные коэффициенты весов сверточных матриц слоя, можно использовать функцию show_kernel:
```
...
![6_conv](img/discrete_conv/6.png)
...
```
![7_conv](img/discrete_conv/7.png)
...
```
![8_conv](img/discrete_conv/8.png)
...
Использовалась следующая структура нейронной сети: [структура_нейронной_сети](https://www.kaggle.com/code/abeerelmorshedy/breast-cancer-using-cnn)
...
Размер входных изображений 50x50. Установим ядра размерности 25x25 с шагом 12. Генерация ядер аналогична примеру с черно-белыми изображениями.
...
```
![9_conv](img/discrete_conv/9.png)
